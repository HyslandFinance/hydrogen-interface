...
import { DAI, USDC_ETHEREUM } from 'constants/tokens'
...
import { useBestMarketOrder } from './useBestMarketOrder'
import { useClientSideV3Trade } from './useClientSideV3Trade'
import useDebounce from './useDebounce'
import useIsWindowVisible from './useIsWindowVisible'

const USDCAmount = CurrencyAmount.fromRawAmount(USDC_ETHEREUM, '10000')
...
describe('#useBestV3Trade ExactIn', () => {
  it('does not compute routing api trade when routing API is not supported', async () => {
    ...
    const { result } = renderHook(() => useBestMarketOrder(TradeType.EXACT_INPUT, USDCAmount, DAI))
  ...
  describe('when routing api is in error state', () => {
    it('does not compute client side v3 trade if routing api is INVALID', () => {
      ...
      renderHook(() => useBestMarketOrder(TradeType.EXACT_INPUT, USDCAmount, DAI))
...
describe('#useBestV3Trade ExactOut', () => {
  it('does not compute routing api trade when routing API is not supported', () => {
    ...
    const { result } = renderHook(() => useBestMarketOrder(TradeType.EXACT_OUTPUT, DAIAmount, USDC_ETHEREUM))
    ...
    expect(mockUseClientSideV3Trade).toHaveBeenCalledWith(TradeType.EXACT_OUTPUT, DAIAmount, USDC_ETHEREUM)
    expect(result.current).toEqual({ state: TradeState.VALID, trade: undefined })
  })
  ...
  describe('when routing api is in error state', () => {
    it('computes client side v3 trade if routing api is INVALID', () => {
      ...
      renderHook(() => useBestMarketOrder(TradeType.EXACT_OUTPUT, DAIAmount, USDC_ETHEREUM))
