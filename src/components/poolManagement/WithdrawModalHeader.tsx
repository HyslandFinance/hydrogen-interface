...
import { AdvancedWithdrawDetails } from './AdvancedWithdrawDetails'
...
export default function WithdrawModalHeader({
  ...
  const { pairs, withdraws } = currentState
    ...
    <AutoColumn gap="4px" style={{ marginTop: '1rem' }}>
      ...
      <div>
        <div style={{margin:"0.5rem 0 0.5rem 0.25rem"}}>
          <Trans>Withdraws</Trans>
        </div>
        {withdraws2.map((withdraw:any,withdrawIndex:number) => {
          const token = currenciesById[withdraw.currencyId]
          const parsedAmountWithdraw = tryParseCurrencyAmount(withdraw.typedAmount||"0", token)
          const fiatValue = fiatValues[withdrawIndex]
          //console.log(`withdraw ${withdrawIndex}`, {withdraw, token, parsedAmountWithdraw, fiatValue: fiatValue, fiatValues})
          return token && (
                  ...
                  <RowBetween align="flex-end">
                    <RowFixed gap="0px">
                      <TruncatedText fontSize={24} fontWeight={500}>
                        {parsedAmountWithdraw ? parsedAmountWithdraw.toSignificant(6) : '0'}
                      </TruncatedText>
                    </RowFixed>
                    <RowFixed gap="0px">
    ...
    <AutoColumn gap="4px" style={{ marginTop: '1rem' }}>
      ...
      <div>
        ...
        {pairs.map((pair:any,pairIndex:number) => {
          ...
          //console.log("in Withdraw market header", {baseToken, quoteToken, buyPrice, sellPrice})
