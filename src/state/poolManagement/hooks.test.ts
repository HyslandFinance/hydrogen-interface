...
import { queryParametersToGridOrderState } from './hooks'

describe('hooks', () => {
  describe('#queryParametersToGridOrderState', () => {
    test('ETH to DAI', () => {
      expect(
        queryParametersToGridOrderState(
          parse(
    ...
    test('does not duplicate eth for invalid output token', () => {
      expect(
        queryParametersToGridOrderState(parse('?outputCurrency=invalid', { parseArrays: false, ignoreQueryPrefix: true }))
      ).toEqual({
        [Field.INPUT]: { currencyId: 'ETH' },
