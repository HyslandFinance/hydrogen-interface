...
import { useBestLimitOrder } from 'hooks/useBestLimitOrder'
...
export function useDerivedLimitOrderInfo(): {
  ...
  parsedAmountOutput: CurrencyAmount<Currency> | undefined
  ...
  const parsedAmountInput = useMemo(
    () => tryParseCurrencyAmount(typedValueInput, inputCurrency ?? undefined),
    [inputCurrency, typedValueInput]
  )
  const parsedAmountOutput = useMemo(
    () => tryParseCurrencyAmount(typedValueOutput, outputCurrency ?? undefined),
    [outputCurrency, typedValueOutput]
  )

  const trade = useBestLimitOrder(
    parsedAmountInput,
    parsedAmountOutput
  )
  ...
  const inputError = useMemo(() => {
    ...
    if (!parsedAmountInput || !parsedAmountOutput) {
    ...
    return inputError
  }, [account, currencies, currencyBalances, parsedAmountInput, parsedAmountOutput, to])
  ...
  return useMemo(
    ...
    [currencies, currencyBalances, inputError, parsedAmountInput, parsedAmountOutput, trade, allowedSlippage]
  )
}
...
export function queryParametersToLimitOrderState(parsedQs: ParsedQs): LimitOrderState {
  let inputCurrency = parseCurrencyFromURLParameter(parsedQs.inputCurrency)
  let outputCurrency = parseCurrencyFromURLParameter(parsedQs.outputCurrency)
  const typedValueInput = parseTokenAmountURLParameter(parsedQs.exactAmountInput)
  const typedValueOutput = parseTokenAmountURLParameter(parsedQs.exactAmountOutput)
...
export function useDefaultsFromURLSearch(): LimitOrderState {
  ...
  const parsedLimitOrderState = useMemo(() => {
    return queryParametersToLimitOrderState(parsedQs)
  }, [parsedQs])
  ...
  useEffect(() => {
    ...
    dispatch(
      replaceLimitOrderState({
        typedValueInput: parsedLimitOrderState.typedValueInput,
        typedValueOutput: parsedLimitOrderState.typedValueOutput,
        inputCurrencyId,
        outputCurrencyId,
